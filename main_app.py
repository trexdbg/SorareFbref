...
st.set_page_config(page_title='sobref')
...
cm = sns.light_palette("#4cdf47", as_cmap=True)
...
# image
if qsodejoseoks['data']['player']['pictureUrl'] is None:
    ...
    with row1_1, _lock:
        ...
        st.info('Derniers Scores (index:GW)')
    ...
    with row1_2, _lock:
        ...
        st.warning('Actions attendues VS Réelles (10 gw)')

        dae = round(df_matchs_players[('Attendu', 'xG')].sum() + df_matchs_players[('Attendu', 'xA')].sum(),1)
        dar = df_matchs_players[('Performance', 'Buts')].sum() + df_matchs_players[('Performance', 'PD')].sum()
        diff_d = round(dae - dar,1)

        st.metric('Décisives Actions - Espérées', str(dae))
        st.metric('Décisives Actions - Réelles', str(dar))
        st.metric('Perf Reelle', str(diff_d), str(diff_d))
    ...
    st.subheader("Actions attendues VS Réelles (10 gw)")
    st.write("**Décisives Actions - Espérées** = Cumul des xG + xA depuis 10 matchs")
    st.write("**Décisives Actions - Réelles** = Cumul des actions décisives depuis 10 matchs (buts + passes)")
    ...
    st.write("**manager sorare** : [nunununu](https://sorare.com/gallery/nunununu) - l'arriere train sifflera 3 fois")